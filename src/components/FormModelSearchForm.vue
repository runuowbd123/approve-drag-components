...
export default {
  ...
  methods: {
    handleSubmit(e) {
      ...
      this.formList.filter(item=>{return item.valueToString}).forEach(item=>{
        if(this.formInline[item.name]) {
          this.formInline[item.valueToString[0]] = this.formInline[item.name][0]
          this.formInline[item.valueToString[1]] = this.formInline[item.name][1]
        }
      })
